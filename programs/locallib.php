...
function calculate_student_progress($studentid) {
    global $DB;

    // 1. Obtenha todos os cursos "trilhas" do Moodle
    $all_courses = $DB->get_records('course'); // Isso pode precisar de mais filtros, dependendo de como você define "trilhas"
    $total_courses = count($all_courses);

    // 2. Obter o progresso do aluno
    $completed_courses = $DB->get_records('course_completions', array('userid' => $studentid, 'completionstate' => 1));
    $num_completed_courses = count($completed_courses);

    // 3. Calcule a porcentagem de progresso
    $progress = ($num_completed_courses / $total_courses) * 100;
