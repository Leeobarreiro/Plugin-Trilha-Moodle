...
final class source_approval_reject extends \local_openlms\dialog_form {
    protected function definition() {
        ...
        $mform->addElement('textarea', 'reason', get_string('source_approval_rejectionreason', 'enrol_programs'));

        $mform->addElement('hidden', 'id');
        $mform->setType('id', PARAM_INT);
        $mform->setDefault('id', $request->id);

        $this->add_action_buttons(true, get_string('source_approval_requestreject', 'enrol_programs'));
    }
