...
// Adicione a barra de progresso aqui
if (has_capability('moodle/course:view', $syscontext) || has_capability('moodle/site:config', $syscontext)) {
    echo '<h3>' . get_string('progressbar_title', 'enrol_programs') . '</h3>';  // <-- Adicionado aqui
...
foreach ($allcourses as $course) {
    $progressinfo = new completion_info($course);
    $progress = $progressinfo->get_data($USER, true);
    // Armazene o progresso em algum lugar, talvez em um array.
}
$totalcourses = count($allcourses);
$completedcourses = 0;
foreach ($allcourses as $course) {
    $progressinfo = new completion_info($course);
    if ($progressinfo->is_course_complete($USER->id)) {
        $completedcourses++;
    }
}
$progresspercentage = ($completedcourses / $totalcourses) * 100;
if (has_capability('moodle/course:view', $context) || has_capability('moodle/site:config', $context)) {
